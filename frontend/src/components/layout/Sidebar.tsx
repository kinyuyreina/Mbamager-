...
interface SidebarProps {
  /** Whether the mobile slide-in drawer is open. Ignored on md+ screens. */
  mobileOpen?: boolean;
  onCloseMobile?: () => void;
}
...
export function Sidebar({ mobileOpen = false, onCloseMobile }: SidebarProps) {
  ...
  return (
    <>
      ...
      <aside
        className={`fixed md:relative inset-y-0 left-0 h-screen bg-slate-900 border-r border-slate-800/80 flex flex-col justify-between py-6 transition-all duration-300 z-50 md:z-30 shrink-0 ${
          collapsed ? 'md:w-20' : 'md:w-64'
        } w-64 ${mobileOpen ? 'translate-x-0' : '-translate-x-full md:translate-x-0'}`}
      >
        {/* Brand logo & title (+ mobile close button) */}
        <div>
          <div className="flex items-center justify-between gap-3 px-6 mb-8 h-9">
            <div className="flex items-center gap-3 overflow-hidden">
              ...
              {!collapsed && (
                <span className="text-sm font-extrabold tracking-tight text-slate-100 font-sans md:inline-block">
                  Mbamager
                </span>
              )}
            </div>
            <button
              onClick={onCloseMobile}
              className="md:hidden p-1.5 rounded-lg text-slate-500 hover:text-slate-100 hover:bg-slate-800/60 cursor-pointer"
          ...
          <nav className="flex flex-col gap-1 px-3">
            {navItems.map((item) => (
              ...
              >
                ...
                {collapsed && (
                  <div className="hidden md:block absolute left-16 bg-slate-900 border border-slate-800 text-slate-100 text-[10px] py-1.5 px-3 rounded-lg opacity-0 group-hover:opacity-100 transition-all pointer-events-none whitespace-nowrap z-50 shadow-md">
                    {item.label}
                  </div>
                )}
              </NavLink>
        ...
        <div className="flex flex-col gap-4">
          ...
          {/* Toggle Collapse button (hidden on mobile - drawer always shows full labels) */}
          <button
            onClick={toggleCollapse}
            className="hidden md:flex mx-3 items-center justify-center p-2 rounded-xl bg-slate-950 border border-slate-800/80 hover:bg-slate-800 hover:text-gold-600 dark:hover:text-slate-100 cursor-pointer transition-colors"
